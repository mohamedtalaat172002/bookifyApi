...
{
    internal sealed class PermissionAuthorizationHandler : AuthorizationHandler<permisssionRequirement>
    {
        ...
        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, permisssionRequirement requirement)
        {
            ...
            HashSet<string> permissions = await authorizationService.GetUserPermissions(identityId);

            if (permissions.Contains(requirement.permisssion))
            {
